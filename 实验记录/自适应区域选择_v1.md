...
- 脚本：`脚本草稿/wam_adaptive_region_select.py`。
- 输出：`结果输出/wam_adaptive_region_select/`。
- 主模型：WAM 官方 MIT 权重。
- 图像：WAM 官方 5 张示例图。
- 水印：固定随机 32-bit 消息。
- 固定变量：
...
C:\Users\86155\miniconda3\envs\bamboo\python.exe `
  脚本草稿\wam_adaptive_region_select.py `
  ...
  --out-dir 结果输出\wam_adaptive_region_select `
  --limit 5 `
  --scale 2.5 `
  --area 30 `
  --block-count 5 `
...
- `结果输出/wam_adaptive_region_select/wam_adaptive_region_select_metrics.csv`
- `结果输出/wam_adaptive_region_select/wam_adaptive_region_select_regions.csv`
- `结果输出/wam_adaptive_region_select/wam_adaptive_region_select_summary.csv`
- `结果输出/wam_adaptive_region_select/wam_adaptive_region_select_overview.csv`
