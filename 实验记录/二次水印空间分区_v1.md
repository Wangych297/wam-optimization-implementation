...
- 脚本：`脚本草稿/wam_rewatermarking_eval.py`。
- 输出：`结果输出/wam_rewatermarking/`。
- 主模型：WAM 官方 MIT 权重。
- 图像：WAM 官方 5 张示例图。
- 强度：`scaling_w=2.5`。
- 消息：
...
C:\Users\86155\miniconda3\envs\bamboo\python.exe `
  脚本草稿\wam_rewatermarking_eval.py `
  ...
  --out-dir 结果输出\wam_rewatermarking `
  --limit 5 `
  --scale 2.5
```
...
- `结果输出/wam_rewatermarking/wam_rewatermarking_metrics.csv`
- `结果输出/wam_rewatermarking/wam_rewatermarking_summary.csv`
- `结果输出/wam_rewatermarking/wam_rewatermarking_overview.csv`
