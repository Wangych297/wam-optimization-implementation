...
- 脚本：`src\wam_optimization/wam_practical_transform_modes.py`。
- 输出：`结果输出/wam_practical_transform_modes/`。
- 图像：WAM 官方 5 张示例图。
- 消息：固定随机 32-bit 消息。
- 对比模式：
...
C:\Users\86155\miniconda3\envs\bamboo\python.exe `
  src\wam_optimization\wam_practical_transform_modes.py `
  ...
  --out-dir 结果输出\wam_practical_transform_modes `
  --limit 5
```
...
- `结果输出/wam_practical_transform_modes/wam_practical_transform_modes_metrics.csv`
- `结果输出/wam_practical_transform_modes/wam_practical_transform_modes_summary.csv`
- `结果输出/wam_practical_transform_modes/wam_practical_transform_modes_overview.csv`
