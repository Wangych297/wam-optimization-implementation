...
- 脚本：`脚本草稿/wam_tamper_localization_eval.py`。
- 输出：`结果输出/wam_tamper_localization/`。
- 主模型：WAM 官方 MIT 权重。
- 图像：WAM 官方 5 张示例图。
- 水印：固定随机 32-bit 消息。
- 强度：`scaling_w=2.5`。
...
C:\Users\86155\miniconda3\envs\bamboo\python.exe `
  脚本草稿\wam_tamper_localization_eval.py `
  ...
  --out-dir 结果输出\wam_tamper_localization `
  --limit 5 `
  --scale 2.5
```
...
- `结果输出/wam_tamper_localization/wam_tamper_localization_metrics.csv`
- `结果输出/wam_tamper_localization/wam_tamper_localization_summary.csv`
- `结果输出/wam_tamper_localization/wam_tamper_localization_overview.csv`
