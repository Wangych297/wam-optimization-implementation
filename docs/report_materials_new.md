...
| 15 | 几何攻击 | ✅ | v1 负向（~0.50）→ v2 derotation 上界 1.000 → v3 GPU 盲扫描 20°=0.904 |
...
推理侧最优配置：
  ...
  检测:   多尺度解码（5 scales）+ rep4_interleaved ECC (crop) / rep3 (resize) + 盲角度扫描 (rotate/flip)
...
效果：
  ...
  几何攻击:         0.500 → 0.904     (盲角度扫描, 20°步长, 71ms)
