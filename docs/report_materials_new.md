...
| 15 | 几何攻击 | ❌❌ | 旋转/翻转完全不可恢复（~0.50），需重训 |
| 诊断 | clean accuracy | 修正 | 仅 0.8% 失败，mask 随机性所致，非 P0 |
| 诊断 | saturation | 修正 | 本管线对饱和度天然鲁棒（0/50 受影响） |
...
推理侧最优配置：
  ...
  检测:   多尺度解码（5 scales）+ rep4_interleaved ECC (crop场景) / rep3 (resize场景)

效果：
  center_crop_0.5:  0.502 → 0.99975  (+99.5%)
  random_crop_0.5:  0.511 → 0.815     (+59.5%)
  resize_0.5:       0.865 → 1.000     (完美)
  JPEG Q=5:         0.999 → 1.000     (完美)
  几何攻击:         0.500             (不可解，需重训)

不推荐：DWSF 多区域分散、scaling_w=3.0、三档模式区分、颜色空间切换、bbox 同步
```

### 局限性

- COCO 5000 张（原论文评测规模的 50%）
- 所有优化均为推理侧，未触及模型权重
- 几何攻击（旋转/翻转）纯推理侧无法解决
- COCO 以自然场景为主，不适用于文档/图表图像
...
**强负向。WAM 对几何变换完全不具备鲁棒性，多尺度检测也无法挽救。**

1. 所有旋转/翻转攻击的 accuracy 在 0.48-0.65 之间，等同于随机猜测
2. 多尺度检测对几何攻击毫无帮助（单尺度和多尺度几乎一致）
3. 根因：WAM 的 ViT 骨干未针对旋转不变性训练，位置编码无法适应旋转后的空间结构
4. 这是纯推理侧无法解决的问题，需要重新训练（引入旋转增强或 Swin+可变形卷积架构）
5. 如果应用场景涉及用户截图后旋转、镜像等操作，WAM 当前完全不可用
