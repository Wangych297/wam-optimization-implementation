...
本文档记录了从 5 张精选图到 COCO val2017 全量 5000 张的完整实验历程，包含基线修正和推理侧优化。共 18 个实验（09-18 推理侧 + 补充探索），最终以实验 18 消融的 COCO 5000 无泄漏确凿数据为权威结论。
...
**⚠ 数据泄漏修正**：初期实验（10、11、14）的多尺度候选选择使用 ground truth 消息进行尺度择优，属于信息泄漏（oracle 上界）。该问题已在实验 18 消融中修复——改用检测掩码置信度（mask_preds.mean()）作为无监督候选选择指标。修复后多尺度单独使用的精度大幅下降（需与 ECC 组合），但几何角度扫描基本不受影响（置信度与真值选角 96% 一致）。下表及实验章节中标注"(oracle)"的数据来自修复前版本，标注"(实用)"的数据来自无泄漏修复后版本。**最终结论以实验 18 消融的 COCO 5000 无泄漏数据为准**。

| 实验 | 方向 | 结果 | 核心数据 |
|:---:|------|:---:|------|
| 09 | 覆盖率扫描 | ❌ | 嵌入侧优化无效，瓶颈在检测侧 |
| 10 | 多尺度检测 | ✅ | center_crop: 0.502→0.525 (实用, 消融B_ms确认) |
| 11 | 多尺度+ECC | ✅ | random_crop: 0.511→0.851 (实用, 消融E_ms_ecc确认) |
...
| 18 | 消融实验 | ✅ | **MS+ECC 实用管线: 0.85-0.99**（COCO 5000 无泄漏确认） |

> 补充探索：极端裁剪 ≤0.4 → ~0.52（物理天花板）；高斯噪声 σ≤0.10 → 0.987（非威胁）；极端缩放 ≤0.15 → ~0.56（硬天花板）；Geo 20° COCO 5000 → 0.904（确认 50 图结论）。

### 最终实用管线（COCO 5000 无泄漏确认）

```text
嵌入: single_center_50pct + scaling_w=2.5 + mask_ratio=0.5
检测: 多尺度解码 (5 scales, 置信度选择) + rep4_interleaved ECC (8-bit)
候选选择: 检测掩码置信度 (无监督, 不偷看真值)

E_ms_ecc (COCO 5000, 750 samples):
  center_crop_0.5: 0.992 (95% perfect)     flip_h:        0.882 (17% perfect)
  random_crop_0.5: 0.851 (46% perfect)     resize_0.25:   0.874 (2% perfect)
  rotate_73:       0.998 (99% perfect)     jpeg_q30:      0.997 (99% perfect)
  none:            1.000 (100% perfect)
```
...
> **信息泄漏修复**：本节数据来自 oracle 版本（候选尺度用真值选择）。实用版本数据见实验 18 消融表 B\_ms 列。
...
> **信息泄漏修复说明**：原版多尺度检测使用 ground truth 消息选择最优尺度（oracle 上界）。修复版使用检测掩码置信度（无监督）选择尺度。修复后数据来自实验 18 消融的 B_ms 模式。

| Attack | Single Scale | Multi Scale (oracle) | Multi Scale (实用) |
|--------|:-----------:|:-------------------:|:------------------:|
| center_crop_0.5 | 0.502 | 0.99975 | **0.525** |
| random_crop_0.5 | 0.511 | 0.701 | **0.579** |
| jpeg_q30 | 0.999 | 0.999 | **0.826** |
| none | 1.000 | 1.000 | **0.854** |

### 结论

**正向但单独使用有局限。** 多尺度检测在 oracle 模式下效果显著（center_crop→0.99975），但实用模式下置信度选尺度不可靠，对非裁剪攻击甚至退化（jpeg 0.999→0.826）。必须与 ECC 组合使用才能发挥价值（见实验 11/18）。

---

## 实验 11：多尺度检测 + 纠错码组合（正向）

> **信息泄漏修复**：本节ECC对比数据来自 oracle 版本（候选尺度用真值选择）。实用版本数据见实验 18 消融表 E\_ms\_ecc 列。
...
> **信息泄漏修复**：下表数值来自 oracle 版本（候选尺度用真值选择）。实用版本见底部说明及实验 18 消融表 E_ms_ecc 列。

| Attack | 基线 | Multi-scale | + ECC (oracle) | 实用 (E_ms_ecc) |
|--------|:---:|:---:|:---:|:---:|
| random_crop_0.5 | 0.506 | 0.701 | 0.804 | **0.851** |
| center_crop_0.5 | 0.501 | 0.99975 | 1.000 | **0.992** |
| jpeg_q30 | 0.999 | 0.999 | 1.000 | **0.997** |
| none | 0.999 | 1.000 | 1.000 | **1.000** |
...
> **信息泄漏修复**：本节ECC对比数据来自 oracle 版本。实用版本数据见实验 18 消融表。
...
|--------|:---:|:---:|:---:|:---:|:---:|:---:|
| center_crop_0.5 | 0.501 | 0.525 | 0.525 | 0.983 | **0.992** | 0.992 |
| random_crop_0.5 | 0.506 | 0.579 | 0.579 | 0.911 | **0.851** | 0.851 |
| rotate_73 | 0.432 | 0.469 | 0.860 | 0.999 | **0.998** | 0.998 |
| flip_h | 0.632 | 0.615 | 0.615 | 0.882 | **0.882** | 0.882 |
| resize_0.25 | 0.945 | 0.893 | 0.893 | 0.873 | **0.874** | 0.874 |
| jpeg_q30 | 0.999 | 0.826 | 0.826 | 1.000 | **0.997** | 0.997 |
| none | 0.999 | 0.854 | 0.854 | 1.000 | **1.000** | 1.000 |

**COCO 5000 全量确认（15/15 chunks, 750 samples/cell）。候选选择使用检测掩码置信度（无监督），无信息泄漏。此前报告的 1.000 为 oracle 上界。**
...
- **最终管线（COCO 5000 确认）**：多尺度检测（置信度选尺度）+ rep4_interleaved ECC (8-bit)，嵌入侧 single_center s=2.5。实用性能：center_crop=0.992, random_crop=0.851, rotate=0.998, flip=0.882, resize=0.874, jpeg=0.997, none=1.000。
