...
| 主动篡改定位 | `results_output/tamper_localization` | 定位被主动移除或破坏的区域 |
| 二次授权更新 | `results_output/provenance_update` | 测试再次授权、分区覆盖和信息保留 |
| 多源素材溯源 | `results_output/source_tracing` | 合成图中定位并恢复不同来源信息 |
...
| Direction | Output | Purpose |
| --- | --- | --- |
| 压缩恢复 | `results_output/compression_recovery` | JPEG 压缩后的多分支候选恢复 |
