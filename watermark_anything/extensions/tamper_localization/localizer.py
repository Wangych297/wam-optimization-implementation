...
def summarize(rows, out_dir):
    ...
    summary_path = out_dir / "tamper_localization_summary.csv"
    ...
    overview_path = out_dir / "tamper_localization_overview.csv"
...
def main() -> int:
    ...
    with torch.inference_mode():
        for image_path in image_paths:
            ...
            distributed_q30_mask = area.union_masks(area.distributed_area_masks(img_pt, 30, 5))
            distributed_q50_mask = area.union_masks(area.distributed_area_masks(img_pt, 50, 5))

            schemes = [
                ("single_center_50pct", single_mask, embedded_full * single_mask + img_pt * (1 - single_mask)),
                ("distributed_q30_5block", distributed_q30_mask, embedded_full * distributed_q30_mask + img_pt * (1 - distributed_q30_mask)),
                ("distributed_q50_5block", distributed_q50_mask, embedded_full * distributed_q50_mask + img_pt * (1 - distributed_q50_mask)),
            ]
    ...
    metrics_path = out_dir / "tamper_localization_metrics.csv"
