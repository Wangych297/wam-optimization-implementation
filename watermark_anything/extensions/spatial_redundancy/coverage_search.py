...
def distributed_area_masks(img_pt, total_area_percent, block_count):
...
def write_summaries(rows, out_dir):
    ...
    summary_path = out_dir / "coverage_search_summary.csv"
    ...
    overview_path = out_dir / "coverage_search_overview.csv"
...
def main() -> int:
        ...
        for image_name, img_pt in cached_inputs:
            ...
            for area in args.areas:
                for block_count in args.block_counts:
                    masks = distributed_area_masks(img_pt, area, block_count)
                    mask_union = union_masks(masks)
                    scheme_img = embedded_full * mask_union + img_pt * (1 - mask_union)
                    scheme_name = f"distributed_q{int(area):02d}_{block_count}block"
    ...
    metrics_path = out_dir / "coverage_search_metrics.csv"
